...
e-=l.rects.length}d.append(new Tiles.Template(f,l.numCols,l.numRows))}return d}},resizeColumns:function(){return this.template.numCols},createTile:function(a){return new Tiles.Tile(a.id,a)}});this.grid.updateTiles(h);this.grid.redraw(this.opts.animate_init,this.onresize,this);d.off("infiniteloaded.themify").on("infiniteloaded.themify",function(b,h){a("#infscr-loading").remove();if(0<h.closest(".auto_tiles").length){var k="undefined"!==typeof LayoutAndFilter?LayoutAndFilter.filterActive:!1;k&&(LayoutAndFilter.filterActive=
!1);var m=g[g.length-1],f=c.SetTemplateByCount(d,c.opts.grids[c.opts.default_grid],h.length),f=a.extend(!0,{},f);if(m===f[0]){for(var r=f[0].length-1,l={},t="XYZVTFK".split(""),n=0;n<r;n++){var p=f[0].charAt(n);"."!==p&&(l[p]=t[n])}for(var q in l)f[0]=f[0].replace(new RegExp(q,"g"),l[q]);f[1]&&f[1]===m&&(f[1]=f[0])}for(var u in f)g.push(f[u]);c.curr_large_template=Tiles.Template.fromJSON(g);m=d.children(".post");c.grid.tiles=[];c.grid.updateTiles(m);c.grid.template=c.get_template();c.grid.isDirty=
!0;c.grid.redraw(c.opts.animate_template,c.onresize,c);k&&(LayoutAndFilter.filterActive=k);setClasses(h,e)}}).data("themify_tiles",this);a(window).on("debouncedresize",function(a){a.isTrigger||(c.grid.template=c.get_template(),c.grid.isDirty=!0,c.grid.resize(),c.addClass("tiles_resing"),c.grid.redraw(b.animate_resize,c.onresize,c))})}})})(jQuery);
