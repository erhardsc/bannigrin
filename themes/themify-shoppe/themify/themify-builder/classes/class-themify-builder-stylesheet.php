	...
	public function get_custom_styling($style_id, $mod_name, $settings, $array = false, $format = 'tag', $rules=false) {
		...
		if (!empty($rules) && is_array($rules)) {
			...
			foreach ($rules as $value) {
				...
				} elseif (in_array($value['prop'], array('font-size', 'line-height', 'padding-top', 'padding-right', 'padding-bottom', 'padding-left', 'margin-top', 'margin-right', 'margin-bottom', 'margin-left', 'border-top-width', 'border-right-width', 'border-bottom-width', 'border-left-width', 'column-rule-width', 'column-gap', 'letter-spacing', 'text-transform'))) {
					if(in_array($value['prop'], array('border-top-width', 'border-right-width', 'border-bottom-width', 'border-left-width', 'column-rule-width'))){
						$temp_id = str_replace( '_width', '', $value['id'] );
	...
	public function get_custom_styling_legacy($style_id, $mod_name, $settings, $array = false, $format = 'tag') {
		...
		if(in_array($mod_name,array('row','column','subrow','sub_column'))){
			...
			$styles_selector = array(
				$selector=> array(
						'background_image','background_gradient', 'background_color', 'font_family', 'font_size', 'line_height', 'letter_spacing', 'text_align', 'color', 'padding', 'margin', 'border_top', 'border_right', 'border_bottom', 'border_left', 'text_transform'
				),
				' a' => array(
